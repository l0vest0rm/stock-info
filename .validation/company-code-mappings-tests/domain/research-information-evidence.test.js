// src/modules/research/domain/research-information-evidence.test.mjs
...
// src/modules/research/domain/research-information-evidence.ts
function reusableReferenceFromCandidate(candidate, evidenceReferenceId) {
  return {
    sourceKind: "research_record",
    sourceId: evidenceReferenceId,
    informationId: candidate.informationId,
    versionId: candidate.versionId,
    documentId: candidate.docId,
    url: candidate.sourceUrl ?? candidate.contentUrl ?? void 0,
    title: candidate.title ?? candidate.sourceName ?? `\u4FE1\u606F\u9884\u5904\u7406\u8BB0\u5F55 ${candidate.informationId}`,
    publishedAt: candidate.publishedAt ?? void 0,
...
function assertEvidenceCandidateReview(input) {
  if (!["accepted", "rejected", "needs_evidence"].includes(input.decision)) throw new Error("evidence candidate decision is invalid");
...
async function loadSourceEvidenceReference(db, evidenceReferenceId) {
  const row = await db.prepare(`select reference.evidence_reference_id as evidenceReferenceId,
    ...
    where reference.evidence_reference_id=?`).bind(required(evidenceReferenceId, "evidenceReferenceId")).first();
  return row ? mapSourceEvidenceReference(row) : null;
}
async function refreshResearchInformationEvidenceCandidates(db, securityCode, createdAt = Date.now()) {
  ...
  const records = await db.prepare(`select record.information_id as informationId, record.entity, record.information_type as informationType,
    ...
    order by result.created_at desc, record.sort_order asc, record.information_id asc`).bind().all();
...
async function materializeResearchInformationEvidenceCandidates(db, securityCode, records, createdAt = Date.now(), options = {}) {
  ...
  for (const record of records) {
    ...
    for (const mapping of matching) {
      ...
      if (wasCreated) created += Number(result.meta.changes);
      else existing += 1;
...
async function reviewResearchInformationEvidenceCandidate(db, input) {
  ...
  const review = {
    candidateReviewId: required(input.candidateReviewId, "candidateReviewId"),
    candidateId: candidate.candidateId,
    decision: input.decision,
    reviewNote: required(input.reviewNote, "reviewNote"),
    reviewedBy: required(input.reviewedBy ?? "local-user", "reviewedBy"),
    reviewedAt,
    createdAt: reviewedAt
  };
  assertEvidenceCandidateReview(review);
  const statements = [db.prepare(`insert into research_information_evidence_candidate_reviews (
      candidate_review_id, candidate_id, decision, review_note, reviewed_by, reviewed_at, created_at
    ) values (?, ?, ?, ?, ?, ?, ?)`).bind(review.candidateReviewId, review.candidateId, review.decision, review.reviewNote, review.reviewedBy, review.reviewedAt, review.createdAt)];
  let reusableEvidenceReference = null;
  if (review.decision === "accepted") {
    ...
    statements.push(db.prepare(`insert into research_reusable_evidence_references (
      ...
      ) values (?, ?, ?, ?, ?, ?, 'needs_field_entry', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
      evidenceReferenceId,
      candidate.candidateId,
      review.candidateReviewId,
    ...
    reusableEvidenceReference = {
      evidenceReferenceId,
      candidateId: candidate.candidateId,
      candidateReviewId: review.candidateReviewId,
...
async function candidateById(db, candidateId) {
  const row = await db.prepare("select * from research_information_evidence_candidates where candidate_id=?").bind(required(candidateId, "candidateId")).first();
  return row ? mapCandidate(row) : null;
}
function mapCandidate(row) {
  const targetModule = text(row.target_module);
...
function mapSourceEvidenceReference(row) {
  const reasons = [];
  ...
  const latestDecision = text(row.latestDecision);
  const sourceChainMatches = referenceCandidateId === candidateId && text(row.referenceCandidateReviewId) === acceptedReviewId && text(row.acceptedReviewCandidateId) === candidateId && text(row.referenceSecurityCode) === text(row.candidateSecurityCode) && text(row.referenceTargetModule) === text(row.candidateTargetModule) && text(row.referenceTargetField) === text(row.candidateTargetField) && text(row.referenceInformationId) === text(row.candidateInformationId) && text(row.referenceResultId) === text(row.candidateResultId) && text(row.referenceRunId) === text(row.candidateRunId) && text(row.referenceVersionId) === text(row.candidateVersionId) && text(row.referenceContentHash) === text(row.candidateContentHash) && text(row.referenceDocId) === text(row.candidateDocId) && text(row.informationId) === text(row.candidateInformationId) && text(row.resultId) === text(row.candidateResultId) && text(row.resultRunId) === text(row.candidateRunId) && text(row.resultVersionId) === text(row.candidateVersionId) && text(row.runId) === text(row.candidateRunId) && text(row.runVersionId) === text(row.candidateVersionId) && text(row.versionId) === text(row.candidateVersionId) && text(row.versionDocId) === text(row.candidateDocId) && text(row.versionContentHash) === text(row.candidateContentHash) && text(row.documentId) === text(row.candidateDocId);
  ...
  const status = reasons.includes("source_chain_mismatch") || reasons.includes("processing_result_not_eligible") ? "invalid" : reasons.includes("latest_review_not_accepted") ? "revoked" : reasons.length ? "superseded" : "eligible";
  return {
    evidenceReferenceId: text(row.evidenceReferenceId),
    candidateId,
    candidateReviewId: text(row.referenceCandidateReviewId),
    securityCode: text(row.referenceSecurityCode),
    entity: text(row.entity),
    informationType: text(row.informationType),
    category: text(row.category),
    period: optional(row.period),
    statement: text(row.statement),
    targetModule: text(row.referenceTargetModule),
    targetField: text(row.referenceTargetField),
    fieldStatus: text(row.fieldStatus),
    document: {
      documentId: text(row.documentId),
      versionId: text(row.versionId),
      contentHash: text(row.versionContentHash),
      currentVersionId: text(row.currentVersionId),
      sourceUrl: optional(row.sourceUrl) ?? optional(row.storedSourceUrl),
      contentUrl: optional(row.contentUrl) ?? optional(row.storedContentUrl),
      title: optional(row.title) ?? optional(row.storedTitle),
      sourceName: optional(row.sourceName) ?? optional(row.storedSourceName),
      publishedAt: optional(row.publishedAt) ?? optional(row.storedPublishedAt),
      locator: text(row.locator)
    },
    processing: {
      informationId: text(row.informationId),
      resultId: text(row.resultId),
      runId: text(row.runId),
      model: text(row.model),
      returnedModel: optional(row.returnedModel),
      promptVersion: text(row.promptVersion),
      schemaVersion: text(row.schemaVersion),
      ontologyVersion: text(row.ontologyVersion),
      inputHash: text(row.inputHash),
      runStatus,
      outcome
    },
    review: {
      decision: "accepted",
      reviewNote: text(row.reviewNote),
      reviewedBy: text(row.reviewedBy),
      reviewedAt: number(row.reviewedAt),
      latestCandidateReviewId: latestReviewId,
      latestDecision
    },
    createdAt: number(row.referenceCreatedAt),
...
function number(value) {
  ...
  if (!Number.isFinite(result)) throw new Error("stored evidence timestamp is invalid");
  return result;
}
...
function refreshDb(records = [sourceRecord2]) {
  const inserted = [];
...
function storedCandidate(securityCode = "300308.SZ") {
  return {
    candidate_id: "candidate:shipment:1",
    security_code: securityCode,
    information_id: sourceRecord2.informationId,
    result_id: sourceRecord2.resultId,
    run_id: sourceRecord2.runId,
    version_id: sourceRecord2.versionId,
    content_hash: sourceRecord2.contentHash,
    doc_id: sourceRecord2.docId,
    entity: sourceRecord2.entity,
    information_type: sourceRecord2.informationType,
    category: sourceRecord2.category,
    period: sourceRecord2.period,
    statement: sourceRecord2.statement,
    target_module: "operating_driver",
    target_field: "segment_volume",
    required_fields_json: JSON.stringify(["operatingModelId", "operatingSegmentId", "fiscalYear"]),
    source_url: sourceRecord2.sourceUrl,
    content_url: sourceRecord2.contentUrl,
    title: sourceRecord2.title,
    source_name: sourceRecord2.sourceName,
    published_at: sourceRecord2.publishedAt,
    mapping_config_version: "research-information-evidence-mapping.v1",
    created_at: 100
  };
}
...
function sourceEvidenceRow(overrides = {}) {
  return {
    evidenceReferenceId: "reference:1",
    referenceCandidateId: "candidate:shipment:1",
    referenceCandidateReviewId: "review:accepted:1",
    referenceSecurityCode: "300308.SZ",
    referenceTargetModule: "operating_driver",
    referenceTargetField: "segment_volume",
    fieldStatus: "needs_field_entry",
    referenceInformationId: sourceRecord2.informationId,
    referenceResultId: sourceRecord2.resultId,
    referenceRunId: sourceRecord2.runId,
    referenceVersionId: sourceRecord2.versionId,
    referenceContentHash: sourceRecord2.contentHash,
    referenceDocId: sourceRecord2.docId,
    storedSourceUrl: sourceRecord2.sourceUrl,
    storedContentUrl: sourceRecord2.contentUrl,
    storedTitle: sourceRecord2.title,
    storedSourceName: sourceRecord2.sourceName,
    storedPublishedAt: sourceRecord2.publishedAt,
    locator: "p.10",
    referenceCreatedAt: 200,
    candidateId: "candidate:shipment:1",
    candidateSecurityCode: "300308.SZ",
    candidateTargetModule: "operating_driver",
    candidateTargetField: "segment_volume",
    candidateInformationId: sourceRecord2.informationId,
    candidateResultId: sourceRecord2.resultId,
    candidateRunId: sourceRecord2.runId,
    candidateVersionId: sourceRecord2.versionId,
    candidateContentHash: sourceRecord2.contentHash,
    candidateDocId: sourceRecord2.docId,
    entity: sourceRecord2.entity,
    informationType: sourceRecord2.informationType,
    category: sourceRecord2.category,
    period: sourceRecord2.period,
    statement: sourceRecord2.statement,
    acceptedReviewId: "review:accepted:1",
    acceptedReviewCandidateId: "candidate:shipment:1",
    acceptedDecision: "accepted",
    reviewNote: "\u6838\u9A8C\u901A\u8FC7",
    reviewedBy: "reviewer",
    reviewedAt: 199,
    latestReviewId: "review:accepted:1",
    latestDecision: "accepted",
    informationId: sourceRecord2.informationId,
    informationResultId: sourceRecord2.resultId,
    resultId: sourceRecord2.resultId,
    resultRunId: sourceRecord2.runId,
    resultVersionId: sourceRecord2.versionId,
    outcome: "extracted",
    runId: sourceRecord2.runId,
    runVersionId: sourceRecord2.versionId,
    model: "local-model",
    ...
    runStatus: "succeeded",
    versionId: sourceRecord2.versionId,
    versionDocId: sourceRecord2.docId,
    versionContentHash: sourceRecord2.contentHash,
    sourceUrl: sourceRecord2.sourceUrl,
    contentUrl: sourceRecord2.contentUrl,
    publishedAt: sourceRecord2.publishedAt,
    documentId: sourceRecord2.docId,
    title: sourceRecord2.title,
    sourceName: sourceRecord2.sourceName,
    currentVersionId: sourceRecord2.versionId,
    ...overrides
  };
}
function sourceEvidenceDb(row) {
  return {
    prepare(sql) {
      ...
      return { bind(evidenceReferenceId) {
        return { first: async () => evidenceReferenceId === "reference:1" ? row : null };
      } };
    }
  };
}
test("refresh uses the exact JSON company-code mapping and preserves the complete source chain", async () => {
  ...
  assert.deepEqual(db.queryCodes, [void 0]);
  ...
  assert.equal(values[16], sourceRecord2.sourceUrl);
  assert.equal(values[17], sourceRecord2.contentUrl);
  assert.equal(values[18], sourceRecord2.title);
});
test("unconfigured information categories do not create research evidence candidates", async () => {
  const db = refreshDb([{ ...sourceRecord2, category: "unconfigured_category" }]);
  const result = await refreshResearchInformationEvidenceCandidates(db, "300308.SZ", 100);
...
test("refresh is idempotent and never broadens a candidate to a different security", async () => {
  ...
  assert.deepEqual(db.queryCodes, [void 0, void 0, void 0]);
});
test("acceptance appends a review and creates only a reusable research_record source reference", async () => {
  ...
  const accepted = await reviewResearchInformationEvidenceCandidate(db, {
    candidateReviewId: "review:accepted:1",
    candidateId: "candidate:shipment:1",
    decision: "accepted",
    reviewNote: "\u6765\u6E90\u548C\u5BF9\u8C61\u5DF2\u6838\u5BF9\uFF0C\u4ECD\u5F85\u4EBA\u5DE5\u586B\u5199\u7ECF\u8425\u5B57\u6BB5\u3002",
    reviewedBy: "reviewer",
    reviewedAt: 200,
    evidenceReferenceId: "reference:1",
    expectedSecurityCode: "300308.SZ"
  });
  ...
  assert.equal(accepted.reusableEvidenceReference.sourceReference.informationId, sourceRecord2.informationId);
  assert.equal(accepted.reusableEvidenceReference.sourceReference.versionId, sourceRecord2.versionId);
  assert.equal(accepted.reusableEvidenceReference.sourceReference.documentId, sourceRecord2.docId);
...
test("reviews are append-only and a cross-security review is rejected before writing", async () => {
  ...
  await reviewResearchInformationEvidenceCandidate(db, {
    candidateReviewId: "review:needs-evidence",
    candidateId: "candidate:shipment:1",
  ...
  await reviewResearchInformationEvidenceCandidate(db, {
    candidateReviewId: "review:rejected",
    candidateId: "candidate:shipment:1",
    decision: "rejected",
    reviewNote: "\u6765\u6E90\u65E0\u6CD5\u652F\u6301\u8BE5\u5B57\u6BB5\u3002",
    reviewedBy: "reviewer",
    reviewedAt: 202,
    expectedSecurityCode: "300308.SZ"
  });
  ...
  await assert.rejects(() => reviewResearchInformationEvidenceCandidate(wrongSecurityDb, {
    candidateReviewId: "review:wrong-security",
    candidateId: "candidate:shipment:1",
    decision: "rejected",
    reviewNote: "\u4E0D\u5E94\u8DE8\u8BC1\u5238\u5BA1\u6838\u3002",
    reviewedBy: "reviewer",
    reviewedAt: 203,
    expectedSecurityCode: "000001.SZ"
...
test("source evidence reader preserves eligible, revoked, superseded, and invalid provenance states", async () => {
  ...
  assert.equal(eligible.document.currentVersionId, sourceRecord2.versionId);
