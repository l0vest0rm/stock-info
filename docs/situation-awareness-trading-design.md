...
知识采集在本机由 `scripts/knowledge-ingest-scheduler.mjs` 按 `config/knowledge-processing.json` 的 `automation.cron`（默认每 15 分钟）运行；它执行现有采集和知识处理，再由 Worker 的态势 Cron（每 10 分钟）完成 Evidence 同步。外部抓取需要真实浏览器、Cookie 或许可时，仍应留在本机任务，不放进 Worker 请求路径。
