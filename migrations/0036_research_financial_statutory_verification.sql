-- Field-level statutory verification is deliberately append-only.  A normalized
-- fact is not itself a filing: each observation records the exact primary fact
-- identity and the statutory disclosure evidence (or why no comparison could
-- be made) so later revisions never overwrite a prior audit trail.
...
create table research_financial_statutory_verifications (
  ...
  statutory_provider text not null check (statutory_provider in ('hkex', 'sec')),
  ...
  -- A decisive comparison must be reproducible from one particular statutory
  -- document and field.  An unverified observation may intentionally lack one
  -- of these because it records a retrieval/extraction gap rather than inventing
  -- a comparison result.
  check (
    outcome = 'unverified'
    or (
