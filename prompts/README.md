...
| --- | --- | --- | --- |
| `company/report-analyze-system.md` + `company/report-analyze-user.md` | 公司研报页：读取研报正文后补充年度业绩预测和单一目标价 | `src/modules/company/api/company.routes.ts` 的 `extractCompanyReportAnalysisByLlm()`；公司研报页的预测补全流程 | 直接 LLM |
| `company/news-report-analyze-system.md` + `company/report-analyze-user.md` | 知识库新闻/研报在公司研报页中的“是否为公司研报、未来年度预测和单一目标价”识别；新闻场景额外要求输出 `isCompanyReport` | `src/modules/company/api/company.routes.ts` 的 `extractCompanyNewsReportByLlm()`；`web/src/modules/knowledge/runtime/knowledge-news-runtime.ts` 的报告分析操作也走该 API | 直接 LLM |
| `company/report-discovery-system.md` + `company/report-discovery-user.md` | 公司研报页“搜索近期公开研报” | `src/modules/company/api/company.routes.ts` 的 `submitCompanyReportDiscoveryTask()`；前端入口为 `web/src/modules/company/pages/company-report-page.ts` | taskd ChatGPT |
...
| `research/financial-analysis-system.md` + `research/financial-analysis-user.md` | 公司财务页“深入财务分析” | `src/modules/research/application/research-financial-analysis.ts`；页面组件为 `web/src/modules/company/pages/company-finance-page.ts` | taskd ChatGPT |
...
- `company/news-report-analyze-user.md`：新闻研报抽取不再维护单独用户模板，改为复用 `company/report-analyze-user.md`，新闻特有的 `isCompanyReport` 约束保留在 `company/news-report-analyze-system.md`。
