...
const scopes = [
  ...
  ["knowledge processing tasks", "workflow_tasks"],
...
function taskStates(db) {
  const tables = tableNames(db).filter((table) => /(?:jobs|processing_runs|workflow_tasks|llm_runs|llm_run_artifacts)$/.test(table));
