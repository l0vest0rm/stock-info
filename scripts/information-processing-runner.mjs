...
export async function runJob(job, owner = runnerInstanceId) {
  ...
  const heartbeat = setInterval(() => { void post(`/api/knowledge/processing-jobs/${encodeURIComponent(taskId)}/heartbeat`, { runId: job.runId, runnerInstanceId: owner, attempt: job.attempt }).catch(() => {}); }, 10_000);
  ...
  try {
    ...
    const completed = await post(`/api/knowledge/processing-jobs/${encodeURIComponent(taskId)}/complete`, { request: job.request, text: response.text, raw: response.raw, cached: false, runId: job.runId, runnerInstanceId: owner, attempt: job.attempt });
    ...
    try { await post(`/api/knowledge/processing-jobs/${encodeURIComponent(taskId)}/fail`, { request: job.request, error: message, runId: job.runId, runnerInstanceId: owner, attempt: job.attempt }); }
