 ...
 */
export async function recordResearchOperatingSourceFactBinding(db: Database, input: OperatingSourceFactBindingWrite): Promise<{ state: "saved" | "unavailable"; operatingSourceFactBindingId: string; reason: "storage_not_initialized" | null }> {
...
export async function reviewResearchOperatingSourceFactBinding(db: Database, input: ResearchOperatingSourceFactBindingReview, operatingCompanyId: string): Promise<{ state: "saved" | "unavailable"; operatingSourceFactBindingReviewId: string; reason: "storage_not_initialized" | null }> {
...
export async function loadResearchOperatingSourceFactBindings(db: Database, query: { operatingCompanyId: string | null; limit?: number }): Promise<OperatingSourceFactBindingSection> {
...
async function sourceFact(db: Database, id: string): Promise<{ operatingCompanyId: string; factKind: ResearchOperatingSourceFactKind } | null> {
...
async function modelCompany(db: Database, id: string): Promise<string | null> { const row = await db.prepare(`select company_id from research_operating_models_typed where operating_model_id=?`).bind(required(id, "operatingModelId")).first<Row>(); return row ? text(row.company_id) : null; }
async function targetCompany(db: Database, kind: string, id: string): Promise<string | null> {
...
async function targetBelongsToModel(db: Database, kind: string, id: string, modelId: string): Promise<boolean> {
  const query = kind === "segment_variable"
