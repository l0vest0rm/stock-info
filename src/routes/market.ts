...
import { fetchText, ok, parseJsonOrJsonp } from "../shared/http";
...
marketRoutes.get("/companies/filter", async (c) => {
  ...
  return ok(c, ((await fetchJson(url, "https://data.eastmoney.com/xuangu/")) as any).result ?? {});
});
...
marketRoutes.get("/companies/change", async (c) => {
  ...
  return ok(c, ((await fetchJson(url, "https://quote.eastmoney.com/")) as any).data ?? {});
});
...
marketRoutes.get("/companies/holding/rank", async (c) => {
  ...
  const body = await fetchJson(url, "https://data.eastmoney.com/zlsj/");
  return ok(c, Array.isArray(body) ? body : (body as any).data ?? []);
});

marketRoutes.get("/companies/report/cnt", (c) => {
  return ok(c, {});
});

marketRoutes.get("/index/positionDates", (c) => {
  return ok(c, []);
});

marketRoutes.get("/index/position", (c) => {
  return ok(c, []);
});

async function fetchJson(url: URL, referer: string): Promise<unknown> {
