      ...
      return h('main', { class: 'option-strategy-page' }, [
            ...
            h('div', { class: 'row g-2' }, [
              ...
              h('div', { class: 'col-6 col-md-4 col-lg-2' }, [
                ...
                h('div', { class: 'input-group input-group-sm' }, [
                  h('input', { class: 'form-control text-end', value: spot.value > 0 ? formatNumber(spot.value) : '', placeholder: '自动获取', readonly: true }),
        ...
        h('section', { class: 'border rounded p-3 bg-white mb-3' }, [
          ...
          h('div', { class: 'row g-2 align-items-end' }, [
            ...
            h('div', { class: 'col-6 col-md-2' }, [h('label', { class: 'form-label mb-1' }, '行权价'), h('input', { class: 'form-control form-control-sm text-end', type: 'number', min: '0', step: 'any', value: legDraft.value.strike, onInput: (event: Event) => legDraft.value = { ...legDraft.value, strike: (event.target as HTMLInputElement).value } })]),
            h('div', { class: 'col-6 col-md-2' }, [h('label', { class: 'form-label mb-1' }, '到期日'), h('input', { class: 'form-control form-control-sm', type: 'date', value: legDraft.value.expiration, onInput: (event: Event) => legDraft.value = { ...legDraft.value, expiration: (event.target as HTMLInputElement).value } })]),
            h('div', { class: 'col-6 col-md-2' }, [h('label', { class: 'form-label mb-1' }, '权利金 / 股'), h('input', { class: 'form-control form-control-sm text-end', type: 'number', min: '0', step: 'any', value: legDraft.value.premium, onInput: (event: Event) => legDraft.value = { ...legDraft.value, premium: (event.target as HTMLInputElement).value } })]),
            ...
            h('table', { class: 'table table-sm align-middle mb-0' }, [
              ...
              h('tbody', legs.value.length ? legs.value.map((leg) => {
                ...
                return h('tr', { key: leg.id }, [
                  ...
                  h('td', leg.type.toUpperCase()), h('td', { class: 'text-end' }, formatNumber(leg.strike)), h('td', { class: 'text-end' }, formatNumber(leg.premium)), h('td', leg.expiration),
                  h('td', { class: 'text-end' }, distance === null ? '请填现价' : `${distance >= 0 ? '+' : ''}${formatNumber(distance)}%`), h('td', { class: 'text-end' }, spot.value > 0 ? formatNumber(timeValue) : '请填现价'), h('td', String(daysToExpiry(leg.expiration) ?? '—')), h('td', String(leg.quantity)), h('td', String(leg.multiplier)),
        ...
        h('section', { class: 'mb-3' }, [
          ...
          h('div', { class: 'row g-2' }, [
            ...
            metricCard('标的当前价', spot.value > 0 ? formatNumber(spot.value) : '待输入', underlying.value.code || '请先填写标的代码'),
          ]),
        ...
        h('section', { class: 'mt-4' }, [
          ...
          h('div', { class: 'table-responsive border rounded bg-white' }, [h('table', { class: 'table table-sm align-middle mb-0' }, [
            ...
            h('tbody', history.value.length ? history.value.map((item) => h('tr', { key: item.id }, [h('td', item.name), h('td', `${item.underlying.name ? `${item.underlying.name} · ` : ''}${item.underlying.code}`), h('td', formatNumber(number(item.underlying.spot))), h('td', String(item.legs.length)), h('td', new Date(item.savedAt).toLocaleString('zh-CN')), h('td', { class: 'text-nowrap' }, [h('button', { type: 'button', class: 'btn btn-sm btn-outline-primary me-2', onClick: () => loadStrategy(item) }, '载入'), h('button', { type: 'button', class: 'btn btn-sm btn-outline-danger', onClick: () => deleteStrategy(item.id) }, '删除')])])) : [h('tr', [h('td', { class: 'text-center text-muted py-4', colspan: 6 }, '暂无历史组合。保存后的记录只保存在当前浏览器的 localStorage 中。')])]),
          ])]),
        ]),
      ])
    }
