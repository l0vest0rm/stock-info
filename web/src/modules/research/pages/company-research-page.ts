...
function relativeValuationLedgerCard(section: RecordValue) {
  const items: RecordValue[] = Array.isArray(section.items) ? section.items : [];
  if (!items.length) return h("section", { class: `research-card section-card ${section.availability || "empty"}` }, [h("h2", "主估值原型与辅助相对估值"), h("p", { class: "research-meta" }, "相对估值必须冻结同行范围、逐年输入和六项可比性门禁；不会计算同行平均、市场一致预期、目标价或交易建议。"), h("div", { class: "research-note" }, section.availability === "unavailable" ? `当前受阻：${section.reason || "账本待初始化"}` : "尚无已保存相对估值账本。必须先完成有来源的同行范围与可比性审查。")]);
