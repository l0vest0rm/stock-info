...
const knowledgeNewsTagClassMap: Record<string, string> = {
  ...
  'recommendation:D': 'text-bg-light text-dark border',
}
...
function onKnowledgeNewsTitleClick(event: Event, row: KnowledgeNewsTableRow) {
  ...
  if (remotePdfUrl) {
    window.open(remotePdfUrl, '_blank', 'noopener')
    return
  }
  if (row.docId) {
...
function knowledgeNewsTitleContent(row: KnowledgeNewsTableRow) {
  ...
  const rankTitle = row.rankReasons.length > 0
    ? row.rankReasons.join(' / ')
    : '推荐排序分'
  const rankClass = row.rankScore >= 80
    ? 'text-bg-danger'
    : row.rankScore >= 60
      ? 'text-bg-primary'
      : row.rankScore >= 40
  ...
  return [
    ...
    ...tags.map((tag) => h('span', {
      ...
      title: tag.startsWith('recommendation:') ? row.recommendationReasons.join(' / ') : undefined,
...
const KnowledgeNewsPage = defineComponent({
  ...
  setup() {
    ...
    const tagFilterOptions: KnowledgeNewsFilterOption[] = [
      ...
      { value: 'recommendation:C', label: '推荐C' },
      { value: 'recommendation:D', label: '推荐D' },
    ]
